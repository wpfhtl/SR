...
import models.networks as networks#the part of the network may be very cleary in the previous experiments
...
class SRGANModel(BaseModel):
    def __init__(self, opt):
        ...
        self.netG = networks.define_G(opt).to(self.device)  # G,the architecture of the G network is shown in the network.py to search the architecture.py
        if self.is_train:
        ...
        if self.is_train:
            ##############################################loss##########################################################################################
            ######################################### G pixel loss
            if train_opt['pixel_weight'] > 0:###when the 'pixel_weight'==0,the loss wouldn't conclude the MSE
            ...
            ######################################## G feature loss.   Content loss or may be you can call it Perceptual loss
            if train_opt['feature_weight'] > 0:
                l_fea_type = train_opt['feature_criterion']
                if l_fea_type == 'l1':
                    self.cri_fea = nn.L1Loss().to(self.device)
                elif l_fea_type == 'l2':####the paper use the l2,while here the program use the l1
